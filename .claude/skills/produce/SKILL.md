...
2. **Concept gate** (mandatory): `gpt_image_2` 4-up direction board per
   concept-board-template.md → user picks a frame (= visual foundation + palette).
3. **Storyboard gate** (mandatory): sheet built FROM that frame per
   storyboard-template.md (15-block spec, Visual World LOCKS, 6 panels/15s) →
   user approval — never skip to clips.
4. Generate per higgsfield-studio plumbing: images (`soul_2`/`nano_banana_pro`, 9:16) or
...
5. QA: remotion-video-review (frames) → **virality_predictor** → fix weakest axis →
